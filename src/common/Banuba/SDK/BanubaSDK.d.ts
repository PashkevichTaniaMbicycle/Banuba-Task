...
declare global {
	/** {@link https://developer.mozilla.org/en-US/docs/Web/API/HTMLCanvasElement/captureStream | HTMLCanvasElement.captureStream()} */
	interface HTMLCanvasElement {
		captureStream(frameRate?: number): globalThis.MediaStream;
	}
	/** {@link https://developer.mozilla.org/en-US/docs/Web/API/HTMLMediaElement/captureStream | HTMLMediaElement.captureStream()} */
	interface HTMLMediaElement {
		captureStream(): globalThis.MediaStream;
	}
}
declare global {
...
export declare type ImageSource = HTMLImageElement | HTMLVideoElement | HTMLCanvasElement | ImageData;
export declare type ImageDataLike = Pick<ImageData, "width" | "height" | "data">;
...
export declare class Webcam implements Input {
	...
	[Symbol.asyncIterator]({ horizontalFlip, ...options }?: InputOptions): AsyncGenerator<ImageData, void, unknown>;
	/** Turns off webcam */
	stop(): void;
}
/**
...
export interface Input {
	...
	[Symbol.asyncIterator](options?: InputOptions): AsyncGenerator<ImageData>;
}
/**
...
export declare const utils: {
	toImageData: (source: ImageSource, options?: InputOptions) => Promise<ImageData>;
	createRenderingContext: {
		(canvas: HTMLCanvasElement | OffscreenCanvas, settings?: CanvasRenderingContext2DSettings | WebGLContextAttributes | undefined): RenderingContext;
		(width: number, height: number): RenderingContext;
	};
};
...
declare class FinalizationGroup {
	register(target: EmscriptenObject): void;
	unregister(): void;
}
/**
...
export declare type SDK = Unwrap<typeof instantiate>;
...
export declare type SDKOptions = {
	 ...
	 * ```
	 */
	locateFile?: string | ((fileName: BanubaSDKBinary) => string) | Record<BanubaSDKBinary, string> | Record<Exclude<BanubaSDKBinary, "BanubaSDK.simd.wasm">, string>;
	/**
...
/** @internal */
export declare function createSDK({ clientToken, locateFile, canvas, logger, ...rest }: SDKOptions): Promise<{
	locateFile: (fileName: BanubaSDKBinary) => string;
	/**
	 * Do *not* pass `canvas` here, it *must* be set later to workaround Emscripten memory leaks
	 */
	FinalizationGroup: typeof FinalizationGroup;
	print: (msg: string) => void;
	printErr: (msg: string) => void;
} & EmscriptenModule & typeof BanubaSDK>;
export declare type Unwrap<T> = T extends Promise<infer U> ? U : T extends (...args: any) => Promise<infer U> ? U : T extends (...args: any) => infer U ? U : T;
...
/** @category Core */
export declare type RenderingOptions = Pick<PlayerOptions, "fps"> & InputOptions;
/** @category Core */
...
export declare class Player extends EventTarget implements globalThis.EventTarget {
	 ...
	 * ```
	 */
	use(input: Input, { fps, ...options }?: RenderingOptions): void;
	/**
	...
	protected _pushFrame({ data, width, height }: ImageData): void;
...
export declare class Effect {
	 ...
	 * @example
	 * ```ts
	 * const makeup = new Effect("effects/Makeup.zip")
	 * const file = $("#file-upload").files[0]
	 *
	 * // ...
	 *
	 * await makeup.writeFile("nude_makeup.png", file)
	 * ```
	 */
	 ...
	 * // ...
	 *
	 * makeup.callJsMethod("Eyes.color", JSON.stringify("0 0.2 0.8 0.64"))
	 * ```
	 */
 ...
 *
 * If you'd like to use the {@link VideoRecorder} on {@link https://caniuse.com/?search=mediarecorder | environments which don't support the MediaRecorder API},
 * consider using a global {@link https://github.com/kbumsik/opus-media-recorder#use-opus-media-recorder-only-when-a-browser-doesnt-support-it | polyfill}.
 *
 * @category Output
 */
